...
publishing {
    publications {
        create<MavenPublication>("FunPublication") {
            ...
            pom {
                name.set("fun-gson")
                description.set("Simple Kotlin Wrapper Library for Gson and JsonPath")
                url.set("https://github.com/jasoet/fun-gson")
                licenses {
                    license {
...
bintray {
    ...
    pkg(delegateClosureOf<BintrayExtension.PackageConfig> {
        name = project.name
        desc = "Simple Kotlin Wrapper Library for Gson and JsonPath"
        repo = "fun"
        setLicenses("Apache-2.0")
        vcsUrl = "https://github.com/jasoet/fun-gson"
        issueTrackerUrl = "https://github.com/jasoet/fun-gson/issues"
        setLabels("kotlin", "fun", "dsl")
        publicDownloadNumbers = true
        version(delegateClosureOf<BintrayExtension.VersionConfig> {
            name = "${project.version}"
            vcsTag = "${project.version}"
            desc = "Fun Gson version ${project.version}."
        })
    })
}
